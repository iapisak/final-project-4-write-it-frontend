                ...
                <div className="d-md-flex">
                    <div className="col-md-7 p-0 mr-md-3">
                        { this.state.posts.map((post, index) => {
                            const template = <a className="text-primary" href={`/post/${ post._id }`} style={{ textDecoration: 'none'}}><small>Read more...</small></a>
                            let content 
                            if (post.content.length > 250) {
                                content = `${post.content.substring(0, 250)} `
                            } else content = post.content

                            return  <div key={ post.photo}>
                                            <a className="text-dark lead" href={`/post/${post._id}`} style={{ textDecoration: 'none'}}>
                                                <h4 className="px-3 px-md-0">{ post.title }</h4> 
                                            </a>
                                            <img className="img-fluid" src={ post.photo } alt={ post.title } />
                                            <p className="text-secondary px-3 px-md-0 my-3">{ content } { template }</p>
                                            <div className="d-flex justify-content-end px-3 px-md-0">
                                                <p className="text-secondary mb-2">{ moment(post.date).format('MMM D, YYYY') } | <span className="text-info"><small>{ moment(post.date).fromNow() }</small></span></p>
                                            </div>
                                            <hr className="mt-0"/>
                                    </div> }) }
                    </div>
                    <div className="col-md-5 my-3 m-md-0 bg-light shadow-sm rounded">
                        <div className="blog-post">
                            <h4 className="blog-post-title pt-3">Commented</h4>
                            <hr className="mt-0"/>
